...
def write_image(array, output_path, data_type):
    # 获取数组的形状
    ...
    # 关闭数据集
    del dataset
def darw_out_seg(data_sample, classes,palette):
    base_dir=r"E:\lizhengcan\ISPRS-CD\train_set102\val"
    if not os.path.exists(base_dir):
        os.makedirs(base_dir)

    data = data_sample.pred_sem_seg
    filename=os.path.basename(data_sample.img_path).replace('第二期影像','')
    file_dir=os.path.join(base_dir,filename)

    num_classes = len(list(classes))

    sem_seg = data.cpu().data
    labels = range(num_classes)

    colors = [palette[label][0] for label in labels]

    mask = np.zeros_like(sem_seg, dtype=np.uint8)
    for label, color in zip(labels, colors):
        mask[0,sem_seg[0] == label ] = color
    mask=np.transpose(mask, (1, 2, 0))
    write_image(mask, file_dir, gdal.GDT_Byte)
...
class SegLocalVisualizer(Visualizer):
    ...
    def __init__(self,
        ...
        self.dataset_meta = dict(
            ...
            palette=[[0, 0, 0], [50, 50, 50],[100, 100, 100],[150, 150, 150],[200, 200, 200],[220, 220,220],[250, 250, 250]])
    ...
    def add_datasample(
        ...
        gt_img_data = None
        pred_img_data = None
        darw_out_seg(data_sample,classes,palette)
