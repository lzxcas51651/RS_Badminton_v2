...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        # gt_semantic_seg[gt_semantic_seg != 250] = 0
...
@TRANSFORMS.register_module()
class Load_IS_Annotations(MMCV_LoadAnnotations):
    """Load annotations for semantic segmentation provided by dataset.
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        #     img_bytes, flag='unchanged',
        #     backend=self.imdecode_backend).squeeze().astype(np.uint8)
        gt_semantic_seg=self.read_label_img(results['seg_map_path'].replace('第二期影像', ''))

        # gt_semantic_seg[gt_semantic_seg != 250] = 0
        # gt_semantic_seg[gt_semantic_seg == 250] = 1

        # gt_semantic_seg[gt_semantic_seg != 250] = 0
        gt_semantic_seg[gt_semantic_seg == 50] = 1
        gt_semantic_seg[gt_semantic_seg == 100] = 2
        gt_semantic_seg[gt_semantic_seg == 150] = 3
        gt_semantic_seg[gt_semantic_seg == 200] = 4
        gt_semantic_seg[gt_semantic_seg == 220] = 5
        gt_semantic_seg[gt_semantic_seg == 250] = 6
        ...
        results['gt_seg_map'] = gt_semantic_seg
        results['seg_fields'].append('gt_seg_map')
    def read_label_img(self,filepath):
        label_options = gdal.WarpOptions(
            format='GTiff',
        ...
        temp_file=os.path.join(temp_dir,self.generate_unique_string()+".tif")
        img: gdal.Dataset = gdal.Open(filepath)
        trans_dataset = gdal.Warp(temp_file, img, options=label_options)
        del img
        trans_data=trans_dataset.ReadAsArray().astype(np.uint8)
        del trans_dataset
        # 永久删除文件
        os.remove(temp_file)
        return trans_data
    ...
    def generate_unique_string(self):
        unique_string = str(uuid.uuid4())[:6]  # 可以根据需要调整字符串长度
...
@TRANSFORMS.register_module()
class Load_IS_MultipleRSImageFromFile(BaseTransform):
    """Load two Remote Sensing mage from file.
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        filename = results['img_path']
        filename2 = results['img_path2'].replace('二', '三')
