...
class RSInferencer_twoImage:
    ...
    def run(self,
            ...
            METAINFO,
            output_path: Optional[str] = None,
            if_match:Optional[bool] = None):
        classes=METAINFO["classes"]
        palette=METAINFO["palette"]
        num_classes = len(list(classes))
        labels = range(num_classes)
        colors = [palette[label][0] for label in labels]
        ...
        image1_data=self.read(image1)
        image2_data = self.read(image2)
        if if_match:
            image2_data = match_histograms(image2_data, image1_data, channel_axis=2)
            # print(if_match)
        image_data_he=np.concatenate([image1_data,image2_data],axis=-1)

        self.model=self.model.eval()
        data, _ = _preprare_data(image_data_he, self.model)
        # vie=data["inputs"][0].cpu().numpy()
        with torch.no_grad():
            result = self.model.test_step(data)
        out_seg = result[0].pred_sem_seg.cpu().data.numpy()[0]


        temp_dir=os.path.join(os.getcwd(),"temp")
        if not os.path.exists(temp_dir):
            os.makedirs(temp_dir)
        temp_file=os.path.join(temp_dir,str(uuid.uuid4())[:6]+".tif")

        mask = np.zeros_like(out_seg, dtype=np.uint8)
        for label, color in zip(labels, colors):
            mask[out_seg == label] = color
        mask=np.expand_dims(mask, axis=-1)
        seg_map = image1.create_seg_map(temp_file)
        seg_map.write(mask)
        ...
        trans_dataset = gdal.Warp(output_path, seg_map.dataset, options=label_options)
